...
def make_plot(ds, title, color, correction=1):
	player = LogPlayer(ds)
	analyzer = XYAnalyzer(mapper=lambda e: (e.timestamp.tm_hour + (e.timestamp.tm_min/5*5)/60.0, e.bbox_query.width * e.bbox_query.height))
...
make_plot( CsvFileSource('../../../data/log2_2011_q4_rand_skrm.csv'), 'Pixels per minute, 24-hour period. Service: skaermkort', 'g', SKRM_SCALE )
make_plot( CsvFileSource('../../../data/log2_2011_q4_rand_mat.csv'), 'Pixels per minute, 24-hour period. Service: mat', 'b', MAT_SCALE )
