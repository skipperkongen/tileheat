...
for i in range( len( in_files ) ):
	...
	accumulate( Y_OPT )
	dump_to_file(Y_OPT, 'data/%s-opt.ys' % date_series[i][-1])

	##
	# HEAT
	##

	trad = [CsvFileSorce(file, 'WMS') for file in in_files[:-1]]
	...
	accumulate( Y_HEAT )
	dump_to_file( Y_HEAT, 'data/%s-heat.ys' % date_series[i][-1] )	

	##
	# HEAT-D
	##

	training_heatmap.dissipate(0.05,0.002)
	Y_HEATD = [ opt_heatmap.get_cell(c.resolution, c.row, c.col).value for c in training_heatmap.sorted_cells() ]
	accumulate( Y_HEATD )
	dump_to_file( Y_HEATD, 'data/%s-heatd.ys' % date_series[i][-1])
	...
	accumulate(Y_GEOM)
	dump_to_file (Y_GEOM, 'data/%s-geom.ys' % date_series[i][-1])
